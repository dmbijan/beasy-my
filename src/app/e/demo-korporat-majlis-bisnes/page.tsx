...
export default function EventPortalPage() {
  ...
  const eventData = {
    title: "Majlis Bisnes Korporat",
    eventType: "corporate",
    eventDate: new Date("2025-12-01T09:00:00"),
    venueName: "Kuala Lumpur Convention Centre",
    venueAddress: "Jalan Pinang, 50088 Kuala Lumpur",
    mapsLink: "https://maps.google.com/?q=KLCC",
    modules: {
      rsvp: true,
      seating: true,
      menu: true,
    ...
    angpao: {
      ...
      bankName: "CIMB Bank",
      accountName: "Syarikat Beasy Sdn Bhd",
      duitnowNumber: "011-1234567",
      duitnowName: "Syarikat Beasy Sdn Bhd",
      qrImageUrl: "/duitnow-qr-sample.svg",
    },
  }
      ...
      >
        <GlassCard variant="light" className="mb-6">
          <div className="text-center">
            <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-500/20 border border-indigo-500/30 text-xs font-medium text-indigo-300 mb-3">
        ...
        {eventSlug === 'demo-korporat-majlis-bisnes' && (
          <GlassCard variant="light" glow="amber" className="text-center">
            <div className="flex flex-col items-center gap-3">
              ...
              <Link href="/dashboard/create">
                <button className="px-6 py-2 rounded-xl bg-gradient-to-r from-rose-500 to-indigo-500 text-white text-sm font-medium hover:from-rose-600 hover:to-indigo-600 transition-all">
                  Cipta Acara Percuma
                </button>
              </Link>
            </div>
        ...
        <AnimatePresence mode="wait">
                ...
                <GlassCard variant="light" glow="amber">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-white/60">Buffet majlis korporat</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    {["Buffet Nasi Lemak", "Ayam Bakar", "Salad Segar", "Minuman"].map((menu) => (
              ...
              {eventData.modules.liveWall && (
                <GlassCard variant="light" glow="emerald">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-white">Live Wall</h3>
                      <p className="text-sm text-white/60">Papan tanda digital untuk peserta</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    {["Ahmad", "Siti", "Ali", "Nurul", "Hafiz", "Aisyah"].map((name) => (
                      <div key={name} className="p-3 rounded-xl bg-white/5 border border-white/10 text-center">
          ...
          {activeModule === "rsvp" && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
            >
              <RSVPForm eventId={eventSlug} onSuccess={(data) => console.log(data)} />
            </motion.div>
          )}
